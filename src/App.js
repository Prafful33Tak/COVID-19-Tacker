// UseState = is basically a variable in REACT , its short term memory for react, whose value
...
import "leaflet/dist/leaflet.css"; // leaflet is a javascript library for interactive maps
import numeral from "numeral";

// defining different useStates
function App() {
  // using the useStates hooks
  // this useState changes the value of variable country to the selected country from the dropdown which is by default worldwide
  ...
  useEffect(() => {
    ...
    fetch("https://disease.sh/v3/covid-19/all") // fetching the API which returns all the combined data of deaths, recovered any many more
      ...
      .then((data) => {
        // the data which we got is passed to setCountryInfo
        setCountryInfo(data);
      });
  }, []);
      ...
      fetch("https://disease.sh/v3/covid-19/countries") // returns all the countries wise details of covid related info
        ...
        .then((data) => {
          const countries = data.map((country) => ({
            // lopping through all the countries and storing the name and code of country in countries array
      ...
      .then((data) => {
        ...
        setMapCenter(
          data.countryInfo?.lat
            ...
            : { lat: 20, lng: 77 }
        ); // set the map center to latitude and longitude of that country
      ...
      <div className="app__left">
        ...
        <div className="app__stats">
          ...
          <InfoBox
            ...
            // On clicking on this card it sets the casesType to total deaths and when the casesType
            // is passed into the map it shows all the data according to this
