...
function LineGraph({ casesType = "cases" }) {
  // passing the casetype as props so that we can put them in the geading of chart
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      await fetch("https://disease.sh/v3/covid-19/historical/all?lastdays=120") //fetching the last 120 days data of cases, recovered and deats from the api
        ...
        .then((data) => {
          // mapping the got data to the function which will return an array of datapoints {x,y} where x is the date and y is the cases
        ...
        <Line
          data={{
            datasets: [
              {
                backgroundColor:
                  util.casesTypeColors[casesType].option.fillColor,
                borderColor: util.casesTypeColors[casesType].option.color,
                data: data,
                fill: true,
              },
            ],
